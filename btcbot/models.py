...
class Signal(db.Model):
    ...
    # Outcome (set after candle closes)
    outcome = db.Column(db.String(10), default='PENDING')  # WIN, LOSS, PENDING
    open_price = db.Column(db.Float)
    close_price = db.Column(db.Float)

    # Order tracking
    mode = db.Column(db.String(10), default='shadow')       # live or shadow
    order_id = db.Column(db.String(100))                    # Limitless order ID
...
class Settings(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    mode = db.Column(db.String(10), default='shadow')        # live or shadow
    position_size = db.Column(db.Float, default=10.0)        # USD
    use_martingale = db.Column(db.Boolean, default=False)
    martingale_multiplier = db.Column(db.Float, default=2.0)
    max_contract_price = db.Column(db.Float, default=0.50)   # max $0.50/contract
