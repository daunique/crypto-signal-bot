...
class TickEMAStrategy:
    ...
    NOTE (2026-07-27): the win rate above describes this class's raw
    reading. engine.py's on_decision_tick() now trades the OPPOSITE of
    what evaluate() returns below -- this class itself is unchanged and
    still reports the raw direction untouched. See engine.py's
    invert_direction()/on_decision_tick() and the README for why, and for
    why this project's own backtests (the ~50% pre-barrier accuracy in
    tick_backtest_addendum.md) suggest the traded win rate after inversion
    is more likely to land back near the number above than at its
    complement.
