...
def _discover_slug_via_active_slugs(ticker: str) -> str | None:
    ...
    try:
        ...
        if matches:
            ...
            # Cache market data so fetch_market() can skip the GET /markets/{slug} call
            if market_data and isinstance(market_data, dict):
                _market_cache[slug] = market_data
                logger.debug("[%s] market data pre-cached from active/slugs slug=%s", ticker, slug)
...
def place_live_order(
    ...
    # 2. Fresh slug (always — markets rotate every 15 min)
