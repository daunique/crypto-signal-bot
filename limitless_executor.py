...
# 5-minute equivalent page/group slugs — same naming convention, "5-min"
# instead of "15-min". Used as match hints the same way _KNOWN_SLUGS is for
# 15-min discovery below.
_KNOWN_SLUGS_5MIN: dict[str, str] = {
    "BTC":  "btc-up-or-down-5-min",
    "ETH":  "eth-up-or-down-5-min",
    "SOL":  "sol-up-or-down-5-min",
    "XRP":  "xrp-up-or-down-5-min",
    "BNB":  "bnb-up-or-down-5-min",
    "DOGE": "doge-up-or-down-5-min",
}
...
def _market_timeframe_rank(slug: str, name: str = "", frequency: str = "",
                           sub_frequency: str = "", target_timeframe: str = "15m") -> int:
    """
    Return a preference rank for a market timeframe (lower = more preferred).
    Returns -1 if the market should be rejected entirely.

    target_timeframe: "15m" or "5m" — which one this bot instance is
    currently trying to discover a market for. Only markets matching the
    target are accepted; everything else is rejected, same strict-single-
    timeframe philosophy as before, just parametrized instead of hardcoded.
    """
    combined = (slug + " " + name + " " + frequency + " " + sub_frequency).lower()

    # IMPORTANT: check 15-min BEFORE 5-min — "15-min" contains "5-min" as a substring
    is_15min = ("15-min" in combined or "15min" in combined or "-15-" in combined)
    is_5min  = (not is_15min) and ("5-min" in combined or "5min" in combined or "-5-" in combined)
    is_1min  = ("1-min" in combined or "1min" in combined) and not is_15min

    if target_timeframe == "15m":
        return 0 if is_15min else -1
    if target_timeframe == "5m":
        return 0 if is_5min else -1
    return -1
...
def _is_15min_market(slug: str, name: str = "", frequency: str = "",
    ...
    return _market_timeframe_rank(slug, name, frequency, sub_frequency, target_timeframe="15m") == 0


def _discover_slug_via_active_slugs(ticker: str, target_timeframe: str = "15m") -> str | None:
    """
    ...
    target_timeframe: "15m" or "5m" — which market cadence to discover.
    """
    # Normalise: "SOL-USDT" → "SOL", "SOL" → "SOL"
    base_ticker = ticker.upper().replace("-USDT", "")
    known_page_slug = (_KNOWN_SLUGS if target_timeframe == "15m" else _KNOWN_SLUGS_5MIN).get(base_ticker, "")
        ...
        for entry in entries:
            ...
            if not entry_ticker:
                for child in children:
                    ...
                    rank = _market_timeframe_rank(child_slug, name, freq, sub_freq, target_timeframe=target_timeframe)
                    if rank == -1:
            ...
            # ── Ticker row: has ticker set (may still have children) ─────────────
                ...
                if children:
                    for child in children:
                        ...
                        if not child_slug:
                            continue
                        rank = _market_timeframe_rank(child_slug, target_timeframe=target_timeframe)
                        if rank == -1:
                            continue
                    ...
                    rank = _market_timeframe_rank(entry_slug, name, freq, sub_freq, target_timeframe=target_timeframe)
                    if rank != -1:
...
def discover_slug(symbol: str, timeframe: str = "15m") -> str | None:
    """
    Find the current active market slug for this symbol at the given timeframe.

    Markets rotate every `timeframe` minutes. At the boundary the old
    ...
    max_attempts = 5
    retry_delay  = 2  # seconds between retries at the boundary — matches the
                       # docstring above and scheduler.py's ORDER_RETRY_DELAY;
                       # this was hardcoded to 30 by mistake, which meant a
                       # slug-discovery miss could stall order placement for
                       # up to 2 minutes (4 gaps × 30s) at the exact moment
                       # the bot needs to move fastest — right at the candle
                       # boundary, competing for early liquidity.

    for attempt in range(1, max_attempts + 1):
        slug = _discover_slug_via_active_slugs(ticker, target_timeframe=timeframe)
        if slug:
            _slug_cache[cache_key] = slug
            _slug_cache[symbol] = slug  # legacy cache key, back-compat for 15m-only readers
            return slug

        if attempt < max_attempts:
            logger.info("[%s] no %s market yet (attempt %d/%d) — retrying in %ds",
                        ticker, timeframe, attempt, max_attempts, retry_delay)
            _time.sleep(retry_delay)

    logger.warning("[%s] no active %s market found after %d attempts", ticker, timeframe, max_attempts)
    return None
...
def place_live_order(
    ...
    """
    Place an order on Limitless Exchange — GTC (resting limit) or FOK
    (fill-or-kill "market" order) depending on `order_type`.

    The on-chain order payload (makerAmount/takerAmount/signature) is
    IDENTICAL either way — Limitless's matching engine is what treats a GTC
    order as "rest until filled" vs a FOK order as "fill completely right
    now or cancel entirely, no partial fill". max_contract_price still
    applies as the price for both: for GTC it's the resting limit price,
    for FOK it's the worst acceptable execution price (slippage bound) —
    FOK does not mean "any price", it means "immediately, at up to this
    price, in full, or not at all".
    ...
    Call flow:
      1. Validate env vars
      2. Discover active market slug for this timeframe (fresh every call)
    ...
    if order_type not in ("GTC", "FOK"):
        order_type = "GTC"
    logger.info("[%s/%s] LIVE %s order: dir=%s $%.2f max_price=$%.2f",
                symbol, timeframe, order_type, signal_direction, position_size_usd, max_contract_price)
    ...
    # Cache key includes timeframe now (symbol alone is ambiguous between
    # the 5m and 15m markets for the same pair).
    cache_key = f"{symbol}:{timeframe}"
    if cache_key not in _slug_cache:
        slug = discover_slug(symbol, timeframe=timeframe)  # may wait up to 2.5min for market rotation
    else:
        slug = _slug_cache[cache_key]
        logger.info("[%s/%s] reusing cached slug=%s", symbol, timeframe, slug)
    if not slug:
        return {"success": False,
    ...
    try:
        ...
        if not resp.ok:
            # A response WAS received — the server explicitly processed and
            # rejected this request (bad signature, insufficient balance,
            # invalid price, etc). Retrying is safe: nothing was placed.
            try:
        ...
        # AMBIGUOUS FAILURE — we genuinely don't know whether Limitless's
        # server received and processed this signed order before the
        # connection dropped or the response timed out. Blindly retrying
        # here would sign and submit a SECOND real order for the same
        # intended position if the first one actually went through — a real
        # double-execution risk, not a hypothetical one. `ambiguous: True`
        # tells the caller (scheduler._run_limitless) to stop and alert
        # rather than retry automatically.
        logger.error("[%s] AMBIGUOUS order failure (network) — NOT safe to auto-retry: %s",
...
def place_shadow_order(
    ...
    discovers the real market slug (public, no auth) SYNCHRONOUSLY, so
    market_slug is populated immediately at signal-creation time — same
    pattern polymarket_executor.place_shadow_order already used.

    This matters beyond bookkeeping: job_track_best_dip and
    job_resolve_outcomes both gate on `if sig.market_slug:` before doing
    anything (dip tracking, polling Limitless's own — Chainlink-backed —
    resolution). Without a slug, dip tracking has nothing to track and
    resolution has no choice but to fall straight to the OKX fallback,
    every single time, regardless of Chainlink being available. This was
    previously missing here (unlike the Polymarket side, which already did
    this), and was the actual root cause of both those symptoms.

    order_type is accepted for consistency with place_live_order (so shadow
    mode's logs/history accurately preview what live mode would do) but
    doesn't change simulated behavior — there's no real orderbook to
    partially rest on vs. immediately fill against in shadow mode.
    """
    if order_type not in ("GTC", "FOK"):
        order_type = "GTC"
    slug = None
    cond_id = None
    try:
        slug = discover_slug(symbol, timeframe=timeframe)
        if slug:
            mkt_data = fetch_market(slug)
            if mkt_data:
                cond_id = (mkt_data.get("conditionId") or mkt_data.get("condition_id")
                          or mkt_data.get("ctfConditionId") or mkt_data.get("condId"))
    except Exception as _de:
        logger.warning("[%s/%s] SHADOW slug discovery error: %s", symbol, timeframe, _de)

    price = min(max_contract_price, 0.99)
    size  = round(position_size_usd / price, 4)
    logger.info("[%s/%s] SHADOW %s dir=%s %.4f shares @ $%.4f slug=%s",
                symbol, timeframe, order_type, signal_direction, size, price, slug)
    return {
        "success":            True,
...
def get_limitless_market_url(symbol: str, timeframe: str = "15m") -> str:
    slug = _slug_cache.get(f"{symbol}:{timeframe}", "") or _slug_cache.get(symbol, "")
    fallback_page = "crypto/15-min" if timeframe == "15m" else "crypto/5-min"
    return (f"https://limitless.exchange/markets/crypto/{slug}"
            if slug else f"https://limitless.exchange/{fallback_page}")
...
def _parse_oracle_price(raw_value) -> float | None:
    """
    Parses a price value that could be either a plain decimal string (older
    Pyth-resolved markets, e.g. "2345.00000000") or an 18-decimal-padded
    integer string (current Chainlink-resolved markets, e.g.
    "234500000000000000000" for the same $2345.00) — see get_market_resolution()
    for why there's no field that says which shape a given response uses.
    Detected by magnitude: no real spot price for these pairs is ever
    legitimately in the billions, so a parsed value that large is the
    unscaled 18-decimal form and gets divided down.
    """
...
# ══════════════════════════════════════════════════════════════════════════════
# MARKET RESOLUTION (authoritative WIN/LOSS — replaces OKX-only calculation)
# ══════════════════════════════════════════════════════════════════════════════
# Crypto 15-min markets resolve automatically at deadline against the Pyth
# Network price feed, not against OKX. Pyth aggregates its own venues on its
# own timing, so its exact tick at market open/deadline can differ from OKX's
# candle open/close even though both are "the BTC price" - that mismatch is
# what causes a signal to read WIN on one side and LOSS on the other.
# GET /markets/{slug} carries the actual outcome Limitless paid out on
# (winningOutcomeIndex), so that should win over an OKX-derived guess
# whenever it is available.
# Docs: docs.limitless.exchange/user-guide/market-resolution
#       docs.limitless.exchange/api-reference/markets/get-market

def get_market_resolution(slug: str) -> dict:
    """
    Fresh (uncached) fetch of a market's resolution state.

    GET /markets/{slug} exposes:
      winningOutcomeIndex - 0 -> YES ("UP" token) won
                             1 -> NO  ("DOWN" token) won
                             null/absent -> not resolved yet
      metadata.openPrice  - the oracle baseline price captured at market
                             open. Limitless migrated its crypto markets from
                             Pyth to Chainlink Data Streams — per Limitless's
                             own changelog, Chainlink-resolved markets return
                             this as an 18-decimal-padded integer STRING
                             (e.g. "234500000000000000000" for $2345.00,
                             i.e. divide by 1e18), while older Pyth-resolved
                             markets returned a plain decimal string instead
                             (e.g. "2345.00000000"). There's no separate
                             field in the response indicating which shape a
                             given market uses, so this is detected by
                             magnitude below — no real crypto spot price for
                             these pairs is legitimately in the billions,
                             so a parsed value that large is unambiguously
                             the unscaled 18-decimal form.

    Always hits the network directly instead of reusing fetch_market()'s
    _market_cache, which is populated at order-placement time (market OPEN)
    and would still show winningOutcomeIndex=None long after the market has
    since closed and resolved.

    Returns dict: resolved(bool), winning_index(int|None),
    winning_side("UP"|"DOWN"|None), open_price(float|None),
    market_status(str|None), error(str|None), raw(dict).
    """
    empty = {"resolved": False, "winning_index": None, "winning_side": None,
             "open_price": None, "market_status": None, "raw": {}}
    if not slug:
        return {**empty, "error": "missing slug"}
    try:
        resp = requests.get(f"{API_BASE}/markets/{slug}", timeout=8)
    except Exception as e:
        logger.warning("[RESOLUTION] GET /markets/%s network error: %s", slug, e)
        return {**empty, "error": str(e)}

    if not resp.ok:
        logger.warning("[RESOLUTION] GET /markets/%s -> HTTP %d", slug, resp.status_code)
        return {**empty, "error": f"HTTP {resp.status_code}"}

    try:
        data = resp.json() or {}
    except Exception as e:
        return {**empty, "error": f"bad JSON: {e}"}

    winning_index = data.get("winningOutcomeIndex")
    if winning_index is None:
        winning_index = data.get("winning_outcome_index")  # defensive alt-casing

    winning_side = {0: "UP", 1: "DOWN"}.get(winning_index)

    metadata       = data.get("metadata") or {}
    raw_open_price = metadata.get("openPrice")
    if raw_open_price is None:
        raw_open_price = data.get("openPrice")
    open_price = _parse_oracle_price(raw_open_price)

    market_status = data.get("status")

    logger.info(
        "[RESOLUTION] slug=%s winning_index=%s (%s) open_price=%s (raw=%s) status=%s",
        slug, winning_index, winning_side, open_price, raw_open_price, market_status
    )

    return {
        "resolved":      winning_index is not None,
...
# ══════════════════════════════════════════════════════════════════════════════
# ORDER STATUS  (POST /orders/status/batch — exact order-ID lookup)
# ══════════════════════════════════════════════════════════════════════════════

# Terminal, on-chain-settled: a REAL fill.
_SETTLED_STATUSES    = {"MINED", "CONFIRMED"}
# Terminal, no money moved: NOT a fill.
_NOT_FILLED_STATUSES = {"UNMATCHED", "FAILED", "CANCELED"}
# Non-terminal, still settling: worth a short retry before giving up.
_IN_FLIGHT_STATUSES  = {"PENDING", "MATCHED", "DELAYED", "RETRYING"}


def check_order_status_batch(order_ids: list) -> dict:
    """
    Look up exact execution/settlement status for one or more orders via
    POST /orders/status/batch (HMAC-authed), keyed by the internal order id
    returned from POST /orders at placement time.

    This is the ID-exact counterpart to the slug-fuzzy-matching in
    _check_order_filled_legacy(): it separates a FULL fill from a PARTIAL
    one via execution.totalsRaw.usdNet (actual USDC settled, net of fees)
    instead of only checking whether any buy trade exists for this market.

    Returns dict keyed by order_id, each value: found(bool), matched(bool),
    settlement_status(str), usd_net(float), contracts_net(float),
    tx_hash(str), error(str), raw(dict).
    """
    order_ids = [str(oid) for oid in order_ids if oid and not str(oid).startswith("shadow_")]
    out = {oid: {"found": False, "matched": None, "settlement_status": None,
                 "usd_net": None, "contracts_net": None, "tx_hash": None,
                 "error": None, "raw": {}}
    ...
    payload  = {"items": [{"orderId": oid} for oid in order_ids]}
    body_str = json.dumps(payload, separators=(",", ":"))
    ...
    try:
        ...
        logger.info("[ORDER-STATUS] POST /orders/status/batch -> %d (%d id(s))",
                    resp.status_code, len(order_ids))
        if not resp.ok:
        ...
        for i, item in enumerate(results):
            if not isinstance(item, dict):
                continue
            oid = item.get("orderId") or (order_ids[i] if i < len(order_ids) else None)
            if oid not in out:
                continue
            if item.get("status") not in (None, "found"):
                out[oid]["error"] = item.get("error") or item.get("status")
                continue
            body      = item.get("data", item) or {}
            execution = body.get("execution") or {}
            totals    = execution.get("totalsRaw") or {}

            def _scaled(key):
                try:
                    return float(totals[key]) / 1_000_000 if key in totals else None
                except (TypeError, ValueError):
                    return None
            ...
            out[oid].update({
                ...
                "settlement_status": execution.get("settlementStatus"),
                "usd_gross":         _scaled("usdGross"),
                "usd_net":           _scaled("usdNet"),
                "contracts_gross":   _scaled("contractsGross"),
                "contracts_net":     _scaled("contractsNet"),
                "tx_hash":           execution.get("txHash"),
                "raw":               body,
            })
    except Exception as e:
        logger.warning("[ORDER-STATUS] batch request error: %s", e)
        for oid in order_ids:
            out[oid]["error"] = str(e)
...
# ══════════════════════════════════════════════════════════════════════════════
# ORDER FILL VERIFICATION  (order-ID exact match, with slug-match fallback)
# ══════════════════════════════════════════════════════════════════════════════

def check_order_filled(market_slug: str, order_id: str,
                        intended_usd=None) -> dict:
    """
    Verify whether an order actually executed on Limitless, and how much of
    it filled — the correct source for martingale decisions, since an OKX
    WIN/LOSS says nothing about whether the Limitless position was ever
    taken. GTC limit orders can sit UNMATCHED indefinitely, or match only
    PARTIALLY, if the chosen limit price is tight relative to available
    liquidity before the 15-min candle closes.

    Primary path: POST /orders/status/batch, looked up by the EXACT
    order_id. A few non-terminal statuses (MATCHED/DELAYED/PENDING/
    RETRYING) mean the trade is still settling on-chain; this polls a
    short, bounded number of extra times before giving up, since those
    normally resolve to MINED within a second or two.

    Falls back to the legacy /portfolio/trades slug-match (then
    /markets/{slug}/user-orders) if the order-status endpoint itself is
    unreachable — preserves the previous behaviour as a safety net rather
    than silently reporting "not filled" on a transient API problem.
    Because that fallback path can only confirm "a trade happened", not
    how much, a legacy-confirmed fill is treated as 100% rather than left
    as an unknown partial (which would otherwise wrongly freeze the
    martingale on a trade that, by this fallback's own evidence, genuinely
    went through).

    Args:
        market_slug  — the 15-min market's slug (used only by the fallback)
        order_id     — internal order id from POST /orders
        intended_usd — the position size that was intended (position_size *
                        martingale multiplier); when given, enables
                        fill_ratio / PARTIAL-fill detection

    Returns dict: filled(bool), status(str), fill_ratio(float|None),
    filled_usd(float|None), trade(dict|None), error(str|None).
    """
    if not order_id:
        return {"filled": False, "status": "ERROR", "fill_ratio": None,
                "filled_usd": None, "trade": None, "error": "missing order_id"}

    if str(order_id).startswith("shadow_"):
        return {"filled": False, "status": "SHADOW", "fill_ratio": None,
                "filled_usd": None, "trade": None,
                "error": "shadow order — no on-chain fill to check"}

    last_status = None
    for attempt in range(1, 4):   # up to 3 tries — settlement is normally sub-second
        result = check_order_status_batch([order_id]).get(str(order_id), {})
        if result.get("found"):
            settlement  = result.get("settlement_status")
            last_status = settlement

            if settlement in _SETTLED_STATUSES:
                # Use the GROSS amount (pre-fee) for fill_ratio — intended_usd
                # is the order's pre-fee size (price * size at placement), so
                # comparing it against usd_net (fees already subtracted) would
                # make even a 100%-matched order read as ~1-2% "partial" from
                # the fee alone. usd_net is still reported as filled_usd since
                # that is the actual dollar amount that ended up at risk.
                filled_usd  = result.get("usd_net")
                gross_usd   = result.get("usd_gross")
                ratio_basis = gross_usd if gross_usd is not None else filled_usd
                fill_ratio  = (round(ratio_basis / intended_usd, 4)
                               if (ratio_basis is not None and intended_usd) else None)
                logger.info(
                    "[FILL-CHECK] order=%s settlement=%s usd_gross=%s usd_net=%s intended=%s ratio=%s",
                    order_id, settlement, gross_usd, filled_usd, intended_usd, fill_ratio
                )
                return {"filled": True, "status": settlement,
                        "fill_ratio": fill_ratio, "filled_usd": filled_usd,
                        "trade": result.get("raw"), "error": None}

            if settlement in _NOT_FILLED_STATUSES:
                logger.info("[FILL-CHECK] order=%s settlement=%s -> not filled",
                            order_id, settlement)
                return {"filled": False, "status": settlement,
                        "fill_ratio": 0.0 if intended_usd else None,
                        "filled_usd": 0.0, "trade": None, "error": None}

            if settlement in _IN_FLIGHT_STATUSES and attempt < 3:
                time.sleep(1.5)
                continue
            break
        else:
            break   # order-status endpoint has no record — try the fallback

    logger.info(
        "[FILL-CHECK] order-status inconclusive for order=%s (last_status=%s) — "
        "falling back to /portfolio/trades slug match", order_id, last_status
    )
    legacy = _check_order_filled_legacy(market_slug, order_id)
    if legacy.get("filled") and intended_usd:
        legacy["fill_ratio"] = 1.0
        legacy["filled_usd"] = intended_usd
    else:
        legacy.setdefault("fill_ratio", None)
        legacy.setdefault("filled_usd", None)
    return legacy


def _check_order_filled_legacy(market_slug: str, order_id: str) -> dict:
    """
    Fallback fill-check via GET /portfolio/trades — on-chain source of truth,
    used only when POST /orders/status/batch itself is unreachable.
