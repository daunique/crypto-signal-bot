...
class LifecycleCapture:
    ...
    def write_record(self, record: dict):
        ...
        self._out_fh.flush()
        os.fsync(self._out_fh.fileno())  # Survive a hard kill, not just a
                                           # clean process exit — matters
                                           # specifically for an unattended
                                           # 12-hour mobile run.
...
async def _periodic_status(cap: LifecycleCapture, interval: int = 300):
    """Print a heartbeat every 5 minutes so a long unattended run
    shows visible signs of life in the terminal/log, and so you can
    tell at a glance (without opening the file) whether it's still
    actually capturing ticks or has gone quiet."""
    while not cap.stop_requested and time.time() < cap.run_until:
        await asyncio.sleep(interval)
        remaining_h = max(0, (cap.run_until - time.time()) / 3600)
        cap.log(f"[STATUS] {cap.market_count} markets seen, "
                 f"{cap.tick_count} ticks recorded, "
                 f"{remaining_h:.2f}h remaining, "
                 f"ws_connected={cap.ws_connected}")
