...
## 2026-07-27 (latest): trade execution inverted -- honest note on what that does and doesn't do to the odds

**Requested directly: the bot now trades the opposite of what `TickEMAStrategy` detects.** A raw `UP` reading now places a `LOWER` contract, not `HIGHER`, and vice versa. Implemented in `engine.py`'s `on_decision_tick()` via a small `invert_direction()` helper, not in `strategy.py` -- the strategy module keeps reporting its raw, un-inverted reading, exactly the same split responsibility as the one-off inversion done for the old candle strategy on 2026-07-25 (further below). `Signal.direction` -- and therefore `contract_type`, the barrier's sign, and what `execute()` actually sends to Deriv -- is the traded (post-inversion) direction throughout, so the dashboard, the database, and the real trade always agree with each other. `reason` is now prefixed `[Inverted from UP]`/`[Inverted from DOWN]` so this is never a silent surprise when reviewing signal history later.

**Stated plainly, because it matters:** this repo's own backtests found the raw signal has roughly **50% pre-barrier directional accuracy** -- `backtest_report.md` section 2 measured 50.3%/51.3% with the barrier offset set to zero, and `tick_backtest_addendum.md`'s 1-tick-duration test converged to almost exactly 50.0%; both read this as "no exploitable edge, fixed-volatility random walk." The ~32-34% win rate quoted in `strategy.py`/`config.py` comes from the Higher/Lower barrier's "dead zone" -- price has to clear entry *plus* the configured offset in the called direction, not just land on the predicted side -- and that dead-zone tax falls symmetrically on whichever direction gets called, since no directional bias was found either way.

**What that means here: inverting is not expected, by this project's own numbers, to flip the win rate to its complement (~66-68%).** It's more likely to land back out near the same ~32-34%, because there's no found directional edge to flip -- a barrier that taxes both directions equally doesn't get un-taxed by calling the other side. This exact inverted configuration hasn't itself been backtested (nothing in this repo's backtest artifacts re-ran the search against the inverted series), so this is shipped the same way the fixed-barrier decision immediately below was: at explicit request, to observe live/demo results directly rather than assume an untested inversion helps.

**New:** `invert_direction()` in `engine.py` is a small, dependency-free pure function specifically so it's unit-testable without the async DB/WebSocket stack the rest of the module needs -- see `backend/tests/test_engine.py`. Same limitation as this project's other testing notes: this sandbox had no network access to install `sqlalchemy`/`pydantic-settings`/etc., so it couldn't be executed here end-to-end; the two-branch logic itself was verified by hand in isolation, and the test file is ready to run in an environment with the real dependencies (`pytest`, per Testing below).
