...
def create_app():
    ...
    with app.app_context():
        ...
        # to existing tables. Any new column added to a model must be migrated here
        # explicitly, otherwise commits that include the new column will fail
        # silently (order placed on-chain but signal never saved to DB).
        try:
            from sqlalchemy import text
            with db.engine.connect() as _conn:
                # market_slug — added in v9; needed for auto-claim after WIN
                _conn.execute(text(
                    "ALTER TABLE signals ADD COLUMN market_slug VARCHAR(200)"
                ))
                _conn.commit()
                logger.info("[APP] Migration: added market_slug column to signals")
        except Exception as _e:
            # Column already exists — this is expected on every restart after the first
            if "duplicate column" not in str(_e).lower() and "already exists" not in str(_e).lower():
                logger.warning(f"[APP] Migration warning (market_slug): {_e}")
